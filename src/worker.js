...
const MAX_UPLOAD_REQUEST_SIZE = 26 * 1024 * 1024;
...
async function handleApi(request, env, url) {
  ...
  if (pathname === '/api/staff/auth/login' || pathname === '/api/staff/auth/logout') return json({error:'Staff authentication is provided by Cloudflare Access.'},410);
  ...
  if(downloadMatch&&request.method==='GET'){
    ...
    const doc=await env.DB.prepare('SELECT * FROM documents WHERE id=? AND user_id=? AND object_status=?').bind(downloadMatch[1],user.id,'available').first(); if(!doc)return json({error:'Document not found.'},404);
...
async function handleStaff(request,env,url){
  ...
  if(pathname==='/api/staff/clients'&&request.method==='GET'){
    const r=await env.DB.prepare(`SELECT u.id,u.email,u.name,u.status,u.created_at,u.activated_at,COALESCE(p.onboarding_status,'active') onboarding_status,(SELECT COUNT(*) FROM documents d WHERE d.user_id=u.id AND d.object_status='available') document_count,(SELECT COUNT(*) FROM document_requests q WHERE q.user_id=u.id AND q.status NOT IN ('completed','declined')) open_request_count FROM users u LEFT JOIN client_profiles p ON p.user_id=u.id ORDER BY u.created_at DESC LIMIT 100`).all();
  ...
  if(cm&&request.method==='GET'){
    ...
    const [profile,docs,reqs]=await Promise.all([
      ...
      env.DB.prepare('SELECT id,title,category,mime_type,file_size,created_at,linked_request_id FROM documents WHERE user_id=? AND object_status=? ORDER BY created_at DESC LIMIT 100').bind(cm[1],'available').all(),
      env.DB.prepare('SELECT id,request_type,notes,status,created_at,updated_at FROM document_requests WHERE user_id=? ORDER BY created_at DESC LIMIT 100').bind(cm[1]).all()
    ]);
...
async function uploadDocument(request,env,staffId,forcedClientId='',linkedRequestId=''){
  ...
  const contentLength=Number(request.headers.get('Content-Length')||0);if(contentLength>MAX_UPLOAD_REQUEST_SIZE)return json({error:'Upload request is too large.'},413);
  ...
  if(file.size>MAX_FILE_SIZE)return json({error:'Files must be 25 MiB or smaller.'},413);
...
async function viewStaffDocument(env,id){
  if(!env.CLIENT_FILES)return json({error:'R2 document storage is not bound yet.'},503);
  const doc=await env.DB.prepare('SELECT * FROM documents WHERE id=? AND object_status=?').bind(id,'available').first();
...
async function fulfilRequest(request,env,staffId,requestId){
  ...
  await audit(env.DB,staffId,'request.fulfilled','document_request',requestId,{});
...
async function listRequests(db){
  const r=await db.prepare(`SELECT q.id,q.user_id,q.request_type,q.notes,q.status,q.created_at,q.updated_at,u.email,u.name,(SELECT d.id FROM documents d WHERE d.linked_request_id=q.id AND d.object_status='available' ORDER BY d.created_at DESC LIMIT 1) fulfilled_document_id FROM document_requests q JOIN users u ON u.id=q.user_id ORDER BY CASE q.status WHEN 'new' THEN 0 WHEN 'in_progress' THEN 1 WHEN 'completed' THEN 2 ELSE 3 END,q.updated_at DESC LIMIT 100`).all();
...
async function requireStaff(request,db,env){const email=await verifyAccessToken(request.headers.get('Cf-Access-Jwt-Assertion'),env);if(!email)return null;return db.prepare("SELECT * FROM staff_users WHERE email=? AND status='active'").bind(email).first()}
...
function safeFilename(title,mime){let s=String(title||'document').replace(/["\r\n\\/]/g,'_').trim()||'document';if(!/\.[a-z0-9]{1,8}$/i.test(s)&&String(mime).includes('pdf'))s+='.pdf';return s}
