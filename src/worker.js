...
async function handleApi(request, env, url) {
  ...
  if (pathname === '/api/documents' && request.method === 'GET') {
    const results=await env.DB.prepare('SELECT id,title,category,mime_type,file_size,created_at,linked_request_id FROM documents WHERE user_id=? ORDER BY created_at DESC').bind(user.id).all();
...
async function handleStaff(request,env,url){
  const staff=await requireStaff(request,env.DB);if(!staff)return json({error:'Staff authentication required.'},401);
  const {pathname}=url;
  ...
  if(cm&&request.method==='DELETE') return deleteClient(env,cm[1]);
  ...
  if(fulfil&&request.method==='POST')return fulfilRequest(request,env,staff.id,fulfil[1]);
  const rm=pathname.match(/^\/api\/staff\/requests\/([^/]+)$/);
  if(rm&&request.method==='PATCH')return updateRequestStatus(request,env.DB,rm[1]);
  return json({error:'Staff route not found.'},404);
}

async function createInvitation(request,db,origin){
